...
if submitted and destination:
    with st.spinner("🌍 Planning your amazing vacation... This may take a few minutes."):
        try:
            ...
            tab1, tab2, tab3 = st.tabs(["📋 Overview Report", "🌤️ Weather & Best Time", "🗓️ Day-by-Day Itinerary"])
            ...
            with tab2:
                st.markdown(f"## 🌤️ Weather & Best Time to Visit {destination}")
                # Display the weather task output (3rd task in the crew)
                weather_output = None
            ...
            with tab3:
                if os.path.exists("detailed_itinerary.md"):
                    ...
                    st.markdown(f"## 🗓️ Your {number_of_days}-Day {destination} Itinerary")
                    st.markdown(itinerary_content)
                    st.download_button(
                        label="💾 Download Itinerary",
                        data=itinerary_content,
                        file_name=f"{destination}_{number_of_days}day_itinerary.md",
                        mime="text/markdown"
                    )
                else:
                    st.markdown("## 📋 Result")
